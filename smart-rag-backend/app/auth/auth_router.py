...
def _decode_any_token(creds: HTTPAuthorizationCredentials) -> dict:
    """
    Accept and decode any Bearer JWT (login-scoped or tenant-scoped).
...
def _require_login_token(creds: HTTPAuthorizationCredentials) -> dict:
    ...
    """
    decoded = _decode_any_token(creds)
    if decoded.get("scope") != "login":
        raise HTTPException(status_code=400, detail="Not a login token")
    return decoded


def _user_id_from_payload(decoded: dict) -> uuid.UUID:
    """
    Extract and validate the UUID user id from 'sub'.
    """
    try:
...
def _tenant_name(db: Session, tenant_id_str: str) -> Optional[str]:
    try:
...
def signup(payload: Dict[str, Any] = Body(...), db: Session = Depends(get_db)):
    ...
    Returns: { token, tenant_id, tenant_name, role }
    """
    ...
    if mode == "create_tenant":
        ...
        return {"token": token, "tenant_id": str(tenant.id), "tenant_name": tenant.name, "role": "owner"}
        ...
        return {
            ...
            "tenant_name": _tenant_name(db, tenant_id_str),
            "role": "member",
        }
...
@router.post("/auth/tenants/create")
def create_tenant(
    ...
    Accepts either a login-scoped token (scope=login) or a tenant-scoped token.
    Body: { "name": "<tenant name>" }
    Returns: { token, tenant_id, tenant_name, role }
    """
    decoded = _decode_any_token(creds)
    user_id = _user_id_from_payload(decoded)

    tname = (payload.get("name") or "").strip()
    if not tname:
    ...
    ms = Membership(user_id=user_id, tenant_id=tenant.id, role="owner")
    db.add(ms)
    db.commit()

    # Mint a tenant-scoped token for the new tenant so the UI can switch immediately.
    token = issue_tenant_token(user_id=str(user_id), tenant_id=str(tenant.id), role="owner")
    return {
        "token": token,
...
def login(payload: Dict[str, Any] = Body(...), db: Session = Depends(get_db)):
    ...
    Returns either:
      { token, tenant_id, tenant_name, role }                         # if tenant chosen & membership valid
    ...
    if desired:
        ...
        token = issue_tenant_token(user_id=str(user.id), tenant_id=str(tenant_uuid), role=ms.role)
        return {
            "token": token,
            "tenant_id": str(tenant_uuid),
            "tenant_name": _tenant_name(db, str(tenant_uuid)),
            "role": ms.role,
        }
...
def search_tenants(q: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    ...
    return {"tenants": [{"id": str(t.id), "name": t.name, "slug": t.slug} for t in rows]}
...
def switch_tenant(
    ...
    """
    Exchange the provided bearer (login **or** tenant token) for a tenant-scoped token
    for the requested tenant, as long as the user is a member.
    ...
    ms = (
        db.query(Membership)
        .filter(Membership.user_id == user_id, Membership.tenant_id == tenant_uuid)
        .first()
    )
    if not ms:
        raise HTTPException(status_code=403, detail="Not a member of requested tenant")

    new_token = issue_tenant_token(user_id=str(user_id), tenant_id=str(tenant_uuid), role=ms.role)
    return {
        "token": new_token,
        "tenant_id": str(tenant_uuid),
        "tenant_name": _tenant_name(db, str(tenant_uuid)),
